/******************************************************************************
 * @file    uds.h
 * @brief   UDS 协议栈应用层头文件，基于 ISO 14229
 * @author  Xin.Gao
 * @date    2026-05-13
 *****************************************************************************/
...
typedef struct __UDS_SERVICE_T
{
    uint8_bl uds_sid;									/* 服务 ID */
	void (* uds_service)(const uint8_bl *, uint16_bl);  /* 服务处理函数 */
	bool_bl (* check_len)(const uint8_bl *, uint16_bl); /* 检查数据长度是否合法 */
	bool_bl std_spt;                                    /* 是否支持默认会话 */
	bool_bl prog_spt;                                   /* 是否支持编程会话 */
	bool_bl ext_spt;                                    /* 是否支持扩展会话 */
	bool_bl fun_spt;                                    /* 是否支持功能寻址 */
	bool_bl ssp_spt;                                    /* 是否支持肯定响应抑制 */
...
* 其它说明: 无
******************************************************************************/
void uds_negative_rsp(uint8_bl sid, uds_nrc_em rsp_nrc);
...
* 其它说明: 无
******************************************************************************/
void uds_positive_rsp(uint8_bl *data, uint16_bl len);
...
* 其它说明: 无
******************************************************************************/
int32_bl uds_timer_chk(uds_timer_t num);

...
static uint8_bl req_seed = 0;				// 接收到请求种子标志
static uint8_bl org_seed_buf[UDS_SEED_LENGTH];
...
/******************************************************************************
* 函数名称: static uint8_bl rand_u8 (void)
* 功能说明: 获取随机数
* 输入参数: 无
* 输出参数: 无
* 函数返回: 8 位随机数
* 其它说明: 无
******************************************************************************/
static uint8_bl rand_u8(void)
{
	static uint32_bl tmp = 666;
	srand(GetMainLoopCnt() + tmp++);
	return (rand() % 0xFF);
}


/******************************************************************************
* 函数名称: int uds_security_access(uint8_bl* key_buf, uint8_bl* seed_buf)
* 功能说明: 比较自己根据种子 seed 计算的 key 值与接收到的 key 值是否一致
* 输入参数: uint8_bl* key_buf		--接收到的 key
    　　　　uint8_bl* seed_buf		--种子
* 输出参数: 无
* 函数返回: 0: 一致; -1: 不一致
* 其它说明: 无
******************************************************************************/
int uds_security_access(uint8_bl* key_buf, uint8_bl* seed_buf)
{
	uint16_bl i = 0;
...
/******************************************************************************
* 函数名称: bool_bl service_27_check_len(const uint8_bl* msg_buf, uint16_bl msg_dlc)
...
* 其它说明: 无
******************************************************************************/
bool_bl service_27_check_len(const uint8_bl* msg_buf, uint16_bl msg_dlc)
{
	bool_bl ret = FALSE;
...
/******************************************************************************
* 函数名称: void service_27_SecurityAccess(const uint8_bl* msg_buf, uint16_bl msg_dlc)
* 功能说明: 27 服务 - 安全访问
...
* 其它说明: 无
******************************************************************************/
void service_27_SecurityAccess(const uint8_bl* msg_buf, uint16_bl msg_dlc)
{
    uint8_bl subfunction;
	uint8_bl rsp_buf[8];
	uint16_bl i;
	...
	switch (subfunction)
	{
		...
		case UDS_SEND_KEY:		// 发送密钥
		{
			...
			if (!uds_security_access((uint8_bl *)&msg_buf[2], org_seed_buf))
			{
				rsp_buf[0] = USD_GET_POSITIVE_RSP(SID_27);
				rsp_buf[1] = subfunction;

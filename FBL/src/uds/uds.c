...
// UDS 应用层相关定时计数器
static uint32_bl uds_timer[UDS_TIMER_CNT] = {0};

// 安全访问种子匹配错误次数
extern uint8_bl uds_fsa_cnt;

// 肯定响应抑制标志，在收到的服务中若带有子功能，其子功能的最高位 bit7 表示肯定响应抑制位
// 当其置 1 时，则表示不需要回复肯定响应，只进行服务处理即可
static bool_bl ssp_flg;
...
******************************************************************************/
static int32_bl uds_timer_run(uds_timer_t num)
{
	// 检查参数合法性
...
******************************************************************************/
static void uds_data_indication (uint8_bl* msg_buf, uint16_bl msg_dlc, n_result_t n_result)
{
	uint8_bl i;
	uint8_bl sid;
	uint8_bl ssp;

...
/******************************************************************************
* 函数名称: static void nt_timer_start_wv(nt_timer_t num, uint32_bl value)
* 功能说明: 重新设置定时器的计数值
* 输入参数: nt_timer_t num              --定时器
    　　　　uint32_bl value              --计数值
* 输出参数: 无
...
******************************************************************************/
static void nt_timer_start_wv(nt_timer_t num, uint32_bl value)
{
    // 检查参数合法性
	if (num >= TIMER_CNT) return;
...
/******************************************************************************
* 函数名称: static int recv_singleframe (uint8_bl* frame_buf, uint8_bl frame_dlc)
* 功能说明: 处理单帧
* 输入参数: uint8_bl*    frame_buf       --接收报文帧数据首地址
    　　　　uint8_bl     frame_dlc       --接收报文帧数据长度
* 输出参数: 无
* 函数返回: 0: OK; -1: ERR
* 其它说明: 无
******************************************************************************/
static int recv_singleframe (uint8_bl* frame_buf, uint8_bl frame_dlc)
{
    uint16_bl i, uds_dlc;
    ...
    #else
    {
        ...
        uds_dlc = (uint16_bl(0x0f & frame_buf[0]) << 8 | frame_buf[1])
...
/******************************************************************************
* 函数名称: static int recv_firstframe(uint8_bl* frame_buf, uint8_bl frame_dlc)
* 功能说明: 处理首帧
* 输入参数: uint8_bl*    frame_buf       --接收报文帧数据首地址
    　　　　uint8_bl     frame_dlc       --接收报文帧数据长度
* 输出参数: 无
...
* 其它说明: 无
******************************************************************************/
static int recv_firstframe(uint8_bl* frame_buf, uint8_bl frame_dlc)
{
    uint16_bl i;
    uint16_bl uds_dlc;

    // 首帧 byte0 的低 4 位以及 byte1 共拼成 12 位的有效数据长度，数据长度最大为 4095
    uds_dlc = ((uint16_bl)(frame_buf[0]&0x0f)) << 8;
    uds_dlc |= frame_buf[1];
...
/******************************************************************************
* 函数名称: static int recv_consecutiveframe(uint8_bl* frame_buf, uint8_bl frame_dlc)
* 功能说明: 处理连续帧
* 输入参数: uint8_bl*    frame_buf       --接收报文帧数据首地址
    　　　　uint8_bl     frame_dlc       --接收报文帧数据长度
* 输出参数: 无
...
* 其它说明: 无
******************************************************************************/
static int recv_consecutiveframe(uint8_bl* frame_buf, uint8_bl frame_dlc)
{
    uint8_bl cf_sn;      // 连续帧的帧序号
	uint16_bl i;
...
/******************************************************************************
* 函数名称: static int recv_flowcontrolframe(uint8_bl* frame_buf, uint8_bl frame_dlc)
* 功能说明: 处理流控帧
* 输入参数: uint8_bl*    frame_buf       --接收报文帧数据首地址
    　　　　uint8_bl     frame_dlc       --接收报文帧数据长度
* 输出参数: 无
...
* 其它说明: 无
******************************************************************************/
static int recv_flowcontrolframe(uint8_bl* frame_buf, uint8_bl frame_dlc)
{
    uint8_bl fc_fs;
...
{
    uint16_bl i;
    uint8_bl send_buf[FRAME_SIZE] = {0};
...
/******************************************************************************
* 函数名称: static int send_singleframe (uint8_bl* msg_buf, uint16_bl msg_dlc)
* 功能说明: 发送单帧
* 输入参数: uint8_bl*    msg_buf         --发送数据首地址
    　　　　uint8_bl     msg_dlc         --发送数据长度
* 输出参数: 无
* 函数返回: 0: OK; -1: ERR
* 其它说明: 无
******************************************************************************/
static int send_singleframe (uint8_bl* msg_buf, uint16_bl msg_dlc)
{
	uint16_bl i;
    uint8_bl send_buf[FRAME_SIZE] = {0};
    ...
    #if(FRAME_SIZE <= 8)
    {
        ...
        send_buf[0] = NT_SET_PCI_TYPE_SF((uint8_bl)msg_dlc);
...
/******************************************************************************
* 函数名称: static int send_firstframe (uint8_bl* msg_buf, uint16_bl msg_dlc)
* 功能说明: 发送首帧
* 输入参数: uint8_bl*    msg_buf         --发送数据首地址
    　　　　uint8_bl     msg_dlc         --发送数据长度
* 输出参数: 无
...
* 其它说明: 无
******************************************************************************/
static int send_firstframe (uint8_bl* msg_buf, uint16_bl msg_dlc)
{
	uint16_bl i;
    uint8_bl send_buf[FRAME_SIZE] = {0};
    ...
    send_buf[0] = NT_SET_PCI_TYPE_FF((uint8_bl)(msg_dlc >> 8));
	send_buf[1] = (uint8_bl)(msg_dlc & 0x00ff);
...
/******************************************************************************
* 函数名称: static int send_consecutiveframe(uint8_bl* msg_buf, uint16_bl msg_dlc, uint8_bl frame_sn)
* 功能说明: 发送首帧
* 输入参数: uint8_bl*    msg_buf         --发送数据首地址
    　　　　uint8_bl     msg_dlc         --发送数据长度
    　　　　uint8_bl     frame_sn        --连续帧帧序号
* 输出参数: 无
* 函数返回: 成功发送的有效数据长度
* 其它说明: 无
******************************************************************************/
static int send_consecutiveframe(uint8_bl* msg_buf, uint16_bl msg_dlc, uint8_bl frame_sn)
{
	uint16_bl i;
    uint8_bl send_buf[FRAME_SIZE] = {0};
...
/******************************************************************************
* 函数名称: static int send_multipleframe (uint8_bl* msg_buf, uint16_bl msg_dlc)
* 功能说明: 发送多帧
* 输入参数: uint8_bl*    msg_buf         --发送数据首地址
    　　　　uint8_bl     msg_dlc         --发送数据长度
* 输出参数: 无
...
* 其它说明: 无
******************************************************************************/
static int send_multipleframe (uint8_bl* msg_buf, uint16_bl msg_dlc)
{
	uint16_bl i;
	uint8_bl send_len;
...
/******************************************************************************
* 函数名称: int network_send_udsmsg(uint8_bl* msg_buf, uint16_bl msg_dlc)
* 功能说明: 发送数据
* 输入参数: uint8_bl*    msg_buf         --发送数据首地址
    　　　　uint8_bl     msg_dlc         --发送数据长度
* 输出参数: 无
...
******************************************************************************/
int network_send_udsmsg(uint8_bl* msg_buf, uint16_bl msg_dlc)
{
    // 检查参数合法性
...
/******************************************************************************
* 函数名称: uds_tp_recv_frame(uint8_bl func_addr, uint8_bl* buf, uint8_bl len)
* 功能说明: 接收到一帧报文并处理
* 输入参数: uint8_bl     func_addr       --0:物理寻址; 1:功能寻址
...
******************************************************************************/
void uds_tp_recv_frame(uint8_bl func_addr, uint8_bl* frame_buf, uint8_bl frame_dlc)
{
    uint8_bl pci_type;
    int ret = -1;
